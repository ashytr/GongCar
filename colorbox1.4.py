...
TARGET_ANGLE1 = 0  # 目标角度（度）
ANGLE_TOLERANCE1 = 0.1  # 角度允许的误差范围
TARGET_DISTANCE1 = 251  # 目标距离（像素）
DISTANCE_TOLERANCE1 = 2  # 距离允许的误差范围
TARGET_ANGLE2 = 0  # 目标角度（度）
ANGLE_TOLERANCE2 = 0.1  # 角度允许的误差范围
TARGET_DISTANCE2 = 251  # 目标距离（像素）
DISTANCE_TOLERANCE2 = 2  # 距离允许的误差范围
# 在全局变量区域添加
last_target_x = None
...
# 使用固定的ROI点位
ROI_DEFAULT_POINTS = {
    'top_left': (160, 120),'top_right': (480, 120),
...
COMMANDS = {
    ...
    b'Q': 'QR',       # ASCII 82 - QR码genshui跟随
    b'q': 'QR_FOLLOW',       # ASCII 82 - QR码shibie
...
def process_ring(frame, direction=None, color_thresholds=RING_COLORS):
    """处理圆环校准，必须提供direction参数
    frame: 输入图像
    direction: 运动方向
    color_thresholds: 颜色阈值字典，默认使用RING_COLORS，可选BOX_COLORS
    """
    ...
    for color_name in ['red', 'green', 'blue']:
        lower, upper = color_thresholds[color_name]
        mask = cv2.inRange(hsv, lower, upper)
        # 边缘检测
...
def process_line(frame, target_direction=None, use_gray=False, target_angle=0, target_distance=251, angle_tolerance=0.1, distance_tolerance=2):
    """处理直线检测
    target_direction: 目标边线方向（A/B/C/D）
    use_gray: 是否使用灰度图处理方式
    target_angle: 目标角度（度）
    target_distance: 目标距离（像素）
    angle_tolerance: 角度允许的误差范围
    distance_tolerance: 距离允许的误差范围
    """
    global last_line_command_time
    ...
    # 使用霍夫直线检测
    lines = cv2.HoughLines(edges, 1, np.pi/1800, 120)
        ...
        if ser2.in_waiting > 0:
            ...
            if data in COMMANDS:
                if data == b'O':
                    ...
                    qr_result = process_qr_follow(frame,'j')
                    if qr_result:
                        ser2.write(f"q{qr_result}\r\n".encode())
                ...
                elif data == b'e':  # wuliao圆环校准命令
                    ring_result = process_ring(frame, 'G', color_thresholds=BOX_COLORS)
                    if ring_result:
                        print(f"发送圆环校准命令: {ring_result}")  # 添加此行打印命令
                        ser2.write(f"{ring_result}\r\n".encode())
                elif data == b'P':  # zhixian校准命令
                    ring_result = process_line(frame, 'D', use_gray=False, target_angle=0, target_distance=100, angle_tolerance=0.1, distance_tolerance=2)
                    if ring_result:
                        print(f"发送zhixian校准命令: {ring_result}")  # 添加此行打印命令
                        ser2.write(f"{ring_result}\r\n".encode())
                elif data == b'Y':  # zhixian校准命令
                    ring_result = process_line(frame, 'C', use_gray=False, target_angle=0, target_distance=100, angle_tolerance=0.1, distance_tolerance=2)
                    if ring_result:
                        print(f"发送zhixian校准命令: {ring_result}")  # 添加此行打印命令
                        ser2.write(f"{ring_result}\r\n".encode())
                elif data == b'X':
