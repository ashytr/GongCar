...
FOLLOW_COMMAND_INTERVAL = 0.5  # 跟随命令发送间隔(秒)
...
def process_follow_block(frame, edge='i'):
    ...
    target_x = 300  # 设置目标位置为280
    tolerance = 10  # 允许的误差范围5
...
"""
    ...
    if use_gray:
        ...
        # 对L通道使用自适应阈值处理
        l_thresh = cv2.adaptiveThreshold(
            l_channel, 255,
        ...
        ab_mask = cv2.inRange(lab, lab_params['gray']['lower'], lab_params['gray']['upper'])
        
        # 将L通道阈值结果与ab通道mask进行与操作
        combined_mask = cv2.bitwise_and(l_thresh, ab_mask)
        
        # 高斯滤波和形态学处理
        gray_mask = cv2.GaussianBlur(combined_mask, (5, 5), 0)
        ...
        # 显示处理过程的中间结果
        show_window('L Channel Thresh', l_thresh)
        show_window('AB Mask', ab_mask)
        show_window('Combined Mask', combined_mask)
    ...
    if lines is not None:
        ...
        # 统一使用大写方向进行处理
        target_direction = target_direction.upper()
        if target_direction == 'A':direction = "ad"  # 前边线
        elif target_direction == 'B':direction = "rm"  # 右边线
        elif target_direction == 'C':direction = "rt"  # 后边线
        elif target_direction == 'D':direction = "lm"  # 左边线
        command = f"Z{direction}00020"  # 固定20个脉冲
        print(f"No line detected, moving {direction} 20 pulses")
...
def main():
    ...
    #     print("串口初始化失败")
    #     exit()
    ser2 = safe_serial_init('COM9', 9600)
    ...
    while True:
        ...
        # process_follow_block(frame)
        process_line(frame, target_direction='A', use_gray=False)
        # 处理串口命令
